...
form.addEventListener('submit', function(e){
  // because we are not submitting the form to the server that's why we have to prevent its default setting by using preventDefault() 
  e.preventDefault()
  // console.log('hello');
  const value = parseInt(amount.value);
  // gotcha : although we declared the type is number in html but we still get the string format so we have to use a conversion to integer 
  // console.log(typeof value);

  const random = Math.floor(Math.random()*text.length);

  // will generate just 1 para : empty, -1, >9
